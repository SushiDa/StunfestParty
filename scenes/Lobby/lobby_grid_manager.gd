...
func _build_grid() -> void:
	...
	for i in (emptySlots / 2):
		characters.insert(characters.size() - (row_length - 1), dummy_character);
		characters.insert(row_length - 1, dummy_character);

	_start_position = - (column_offset * (row_length - 1)) / 2.0 - row_offset * (ceilf((1 * chara_local.size() ) / (1.0 * row_length)) - 1) / 2.0;
...
func get_slot_position(index: int) -> Vector2:
	...
	return _start_position + row * row_offset + column * column_offset;
...
func get_next_index(start_index: int, delta: int) -> int:
	...
	if(new_index < 0): new_index += _slots.size()
	new_index %= _slots.size()
